...
Debug = True
alarm_read_command = "FEFE00D7E2FE0D"
alarm_start = b'FEFE'
alarm_stop = b'FE0D'
...
class AlarmInterface:
    ...
    def initialize(self):
        self.alive = True
        self.thread_read = threading.Thread(target=self.reader(alarm_read_command, alarm_start, alarm_stop))
        self.thread_read.setDaemon(True)
        self.thread_read.setName('Alarm Input')
    ...
    def reader(self, command, start, stop):
        ...
        recived_msg=False
        isReceving=False
        if Debug: print("\nAlarmInterface.reader initialization")
        while self.alive:
            #wyslanie komendy do alarma inicjalizujacej odczyt
            self.serial.write(command)
            #print(binascii.hexlify(command[0]))
            print("Send command to alarm:",command)

            #odbior odpowiedzi od alarma
            print("Start reading from alarm:")
            isReceving = True
            licznik=0
            data=''
            while isReceving:
    ...
    def message_analysis(self, text):
        if len(text)==24:#MSMS? nie sprawdzamy czy nie przyszła cała ramka, albo co jak przyjdzie przesunieta?
            ...
            if text[3]=="1":
                command = text[:3]
                if command == '103':
                    self.can_serial_no(item_id, group_id, text)
                elif command == '104':
                    self.can_serial_no(item_id, group_id, text)
                elif command == '308':
                    self.can_status_rgb(item_id, group_id, text, False)
            if text[3]=="0":
                command = text[:3]
                if command == '301':
                    self.can_status_button(item_id, group_id, text, True)
                elif command == '302':
                    self.can_status_relay(item_id, group_id, text, True)
                elif command == '308':
                    self.can_status_rgb(item_id, group_id, text, True)
