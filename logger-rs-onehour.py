...
#log_subfolder = '/media/flash-log/'
log_subfolder = 'logggg'
file_extension = ".txt"
...
def open_file_in(log_path, log_name):
...
def main_loop(input_port, time_end, log_file):
    buffer_string = 0
    previous_datestamp=0
    while (datetime.datetime.today() < time_end):
        actual_datestamp = datetime.datetime.today()
        print(actual_datestamp)
        if ((previous_datestamp + datetime.timedelta(seconds=3)) < actual_datestamp):
            print("previous: {} actual: {}".format(previous_datestamp, actual_datestamp))
            previous_datestamp = actual_datestamp
            print("previous: {} actual: {}".format(previous_datestamp, actual_datestamp))
...
log_path = os.path.join(os.getcwd(),log_subfolder)

file = open_file_in(log_path, log_name)
