...
class CanbusReader:
    ...
    def canbus_status_button(self, item_id, group_id, data, change):
        ...
        try:
            ...
            if device_id and channel <= '09':
                cursor.execute("""UPDATE device_status SET status = %s WHERE device_id = %s and channel_id = %s """, (str(int(status,16)), device_id, channel))
                ...
                if sensor_id:
                    if sensor_scaler == 'NO':
                       cursor.execute("""UPDATE sensor_status SET status = %s WHERE sensor_id = %s and channel_id = %s """, (str(255-int(status,16)), sensor_id, sensor_channel))
                    else:                    
                       cursor.execute("""UPDATE sensor_status SET status = %s WHERE sensor_id = %s and channel_id = %s """, (str(int(status,16)), sensor_id, sensor_channel))
        ...
        if self.spy:
            if channel <= '09':
                ...
                sys.stdout.write('RX: %s,%s - Bledna ramka dla kanalu %d\n' % (group_id, item_id, channel))
            sys.stdout.flush()        
    ...
    def canbus_status_relay(self, item_id, group_id, data, change):
        ...
        try:
            ...
            if device_id and channel <= '09':
                ...
                if actor_id:
                    if actor_scaler == 'NO':
                       cursor.execute("""UPDATE actor_status SET status = %s WHERE actor_id = %s and channel_id = %s """, (str(255-int(status,16)), actor_id, actor_channel))
                    else:                    
                       cursor.execute("""UPDATE actor_status SET status = %s WHERE actor_id = %s and channel_id = %s """, (str(int(status,16)), actor_id, actor_channel))
        ...
        if self.spy:
            if channel <= '09':
                ...
                sys.stdout.write('RX: %s,%s - Bledna ramka dla kanalu %s.\n' % (group_id, item_id, channel))
            sys.stdout.flush()        
    ...
    def canbus_status_dimmer(self, item_id, group_id, data, change):
        ...
        try:
            ...
            if device_id and channel == '01':
                ...
                if actor_id:
                    if actor_scaler == 'NO':
                       cursor.execute("""UPDATE sensor_status SET status = %s WHERE sensor_id = %s and channel_id = %s """, (str(255-int(status,16)), actor_id, actor_channel))
                    else:                    
                       cursor.execute("""UPDATE sensor_status SET status = %s WHERE sensor_id = %s and channel_id = %s """, (str(int(status,16)), actor_id, actor_channel))
        ...
        if self.spy:
            if channel == '01':
                ...
                freq0 = int(data[16:18],16)
                freq1 = int(status,16)
                freq = freq1 * 256 + freq0
                sys.stdout.write('RX: %s,%s - Czestotliwosc napiecia 230V - %s milisekund.\n' % (group_id, item_id, freq))
            sys.stdout.flush()        
...
class CanbusWriter:    
    ...
    def message_analysis(self, command): 
        ...
        if group_id.isdigit() and item_id.isdigit():                    
            ...
            #           x10    x10     xFE      xFE       xFF     xFF                                  xFF   xFF        xFF    xFF
            elif instr == '102':
               if self.spy:
...
#    sql_db   = 'domomatik_central'
    ...
    try:
        mysql = MySQLdb.connect(host = sql_host,port=7770, user = sql_user, passwd = sql_pass, db = sql_db)
    except MySQLdb.Error, e:
