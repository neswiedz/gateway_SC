(function($) {
	$.modal = function(config) {
		...
		if (options.overlayClose) {
			overlay.bind ('click', function (e) { $.modal.close (); options.afterClose(); });
		}
		
		close.bind ('click', function (e) { $.modal.close (); options.afterClose(); });		
						
		(options.title !== '') ? header.append ('<h3>' + options.title + '</h3>') : '';				
		(options.byline !== '') ? header.append ('<div class="byline">' + options.byline + '</div>') : '';	
		
		if (options.ajax !== '') {
			content.html ('<div id="modal_loader"><img src="./public/images/mba/ajax-loader.gif" /></div>');
			$.modal.reposition ();
